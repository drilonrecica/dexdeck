...
tar -xzf dexdeck-0.2.0-x86_64-unknown-linux-gnu.tar.gz
install -m 755 dexdeck-0.2.0-x86_64-unknown-linux-gnu/dexdeck ~/.local/bin/
```

On Windows use `Get-FileHash -Algorithm SHA256`, extract the ZIP, and place
`dexdeck.exe` on `PATH`. Windows support is experimental in 0.2.0.
